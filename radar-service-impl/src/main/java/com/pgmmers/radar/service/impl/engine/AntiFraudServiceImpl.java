...
public class AntiFraudServiceImpl implements AntiFraudService {
    ...
    public Map<String, Object> prepare(Long modelId, Map<String, Object> jsonInfo) {
        List<PreItemVO> preItemList = preItemService.listPreItem(modelId);//根据模型获取需要定义的预处理字段
        Map<String, Object> result = new HashMap<>();
        for (PreItemVO item : preItemList) {
            if (!item.getStatus().equals(StatusType.ACTIVE.getKey())) {
                continue;
            }
            String[] sourceField = item.getSourceField().split(",");  //获取原始字段
            Object transfer = pluginManager.pluginServiceMap(item.getPlugin()).handle(item,jsonInfo,sourceField);  //调用插件进行预处理
